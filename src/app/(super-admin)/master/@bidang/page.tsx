...
import { useBidang, useBidangDelete, usePostBidang, useUpdateBidang } from "@/hooks/fetch/master/useBidang";
...
function TodoPage() {
    ...
    const { mutateAsync : deleteBidang, isPending : deletePending, isError : deleteError, isSuccess : deleteSuccess } = useBidangDelete();
    const { mutateAsync : createBidang, isPending : createPending, isError : createError, isSuccess : createSuccess } = usePostBidang();
    const {mutateAsync : updateBidang, isPending : updatePending, isError : updateError, isSuccess : updateSuccess} = useUpdateBidang();
    ...
    useEffect(() => {
        setTodos(bidang?.data?.bidangTindakLanjut ?? [])
    ...
    const handleDelete = async (id: string) => {
        try {
            await deleteBidang(id); // Trigger the mutation
    ...
    const handleSave = async(id: string, newText: string) => {
        try {
            console.log(newText)
            const bidang = await updateBidang({id : id.toString(), formData : {nama : selectedTodo?.nama}});
            console.log(bidang)
    ...
    return (
        <div className="max-w-xl mx-auto p-4 space-y-1 relative">
            <h1 className="text-2xl font-bold">📝 Master Bidang</h1>
        
            <div className="flex gap-2">
                <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={handleKeyDown} placeholder="tambah bidang baru" className="flex-1 border border-gray-300 px-3 py-2 rounded"/>
                <button onClick={handleAdd}  className="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600">Add</button>
            </div>
        
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="cari bidang..." className="w-full border border-gray-300 px-3 py-2 rounded "/>
