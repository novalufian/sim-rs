...
import { useJabatan, useJabatanDelete, usePostJabatan, useUpdateJabatan } from "@/hooks/fetch/master/useJabatan";
...
function TodoPage() {
    ...
    const { data : jabatan , isLoading : useJabatanLoading} = useJabatan();
    const { mutateAsync : deleteJabatan, isPending : deletePending, isError : deleteError, isSuccess : deleteSuccess } = useJabatanDelete();
    const { mutateAsync : createJabatan, isPending : createPending, isError : createError, isSuccess : createSuccess } = usePostJabatan();
    const {mutateAsync : updateJabatan, isPending : updatePending, isError : updateError, isSuccess : updateSuccess} = useUpdateJabatan();
    ...
    useEffect(() => {
        setTodos(jabatan?.data?.jenisJabatan ?? [])
    ...
    const handleDelete = async (id: string) => {
        try {
            await deleteJabatan(id); // Trigger the mutation
    ...
    const handleSave = async(id: string, newText: string) => {
        try {
            console.log(newText)
            const bidang = await updateJabatan({id : id.toString(), formData : {nama : selectedTodo?.nama}});
    ...
    return (
        <div className="max-w-xl mx-auto p-4 space-y-1 relative">
            <h1 className="text-2xl font-bold flex justify-between items-center">👑 Master Jabtan <span className="text-sm font-light text-gray-500">{todos.length} data</span></h1>
        
            <div className="flex gap-2">
                <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={handleKeyDown} placeholder="Jabatan baru..." className="flex-1 border border-gray-300 px-3 py-2 rounded"/>
