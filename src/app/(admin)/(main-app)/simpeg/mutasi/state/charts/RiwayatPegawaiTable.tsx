...
import { useRiwayatPegawai, StatistikPegawaiFilter } from "@/hooks/fetch/mutasi/useMutasiState";
import SpinerLoading from "@/components/loading/spiner";
import { BsCloudSlash } from "react-icons/bs";

export default function RiwayatPegawaiTable() {
    ...
    const { data, isLoading, isError } = useRiwayatPegawai(searchFilters, true);
    ...
    if (isLoading) {
        return (
            <div className="rounded-4xl bg-white dark:border-gray-800 dark:bg-white/[0.03] box-border p-5 min-h-[360px] h-full flex flex-col items-center justify-center">
                <SpinerLoading title="Riwayat Pegawai" />
            </div>
        );
    }
    ...
    return (
        <div className="rounded-4xl bg-white dark:border-gray-800 dark:bg-white/[0.03] box-border p-5 min-h-[360px] h-full flex flex-col">
            ...
            <h2 className="text-2xl font-semibold mb-6 text-center text-gray-800 dark:text-gray-200">Riwayat Mutasi Pegawai</h2>
                        ...
                        <table className="w-full divide-y divide-gray-200 dark:divide-gray-700">
                            <thead className="bg-gray-100 dark:bg-gray-700">
                                <tr>
                                    ...
                                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-white uppercase">Jenis Mutasi</th>
                                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-white uppercase">Instansi Tujuan</th>
                            ...
                            <tbody className="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
                                {riwayat.data.map((item, index) => (
                                    <tr key={index} className="hover:bg-gray-50 dark:hover:bg-gray-800">
                                        ...
                                        <td className="px-4 py-3 text-sm text-gray-900 dark:text-white">{item.jenis_mutasi}</td>
                                        <td className="px-4 py-3 text-sm text-gray-900 dark:text-white">{item.instansi_tujuan}</td>
