...
import { useStatistikPerTahun, StatistikTahunFilter } from "@/hooks/fetch/mutasi/useMutasiState";
...
export default function StatistikPerTahunChart() {
    ...
    const chartData = {
        ...
        datasets: [
            ...
            {
                label: 'Disetujui',
                data: statistik.per_tahun.map((item) => item.disetujui),
                backgroundColor: 'rgba(16, 185, 129, 0.8)',
            },
            {
                label: 'Ditolak',
                data: statistik.per_tahun.map((item) => item.ditolak),
                backgroundColor: 'rgba(239, 68, 68, 0.8)',
            },
            {
                label: 'Pending',
                data: statistik.per_tahun.map((item) => item.pending),
        ...
        <div className="rounded-4xl bg-white dark:border-gray-800 dark:bg-white/[0.03] box-border p-5 min-h-[360px] h-full flex flex-col">
            ...
            <div className="mt-4 text-center">
                <p className="text-xs text-gray-400 dark:text-gray-500">
                    Periode: {statistik.periode.tahun_mulai} - {statistik.periode.tahun_selesai}
                </p>
            </div>
        </div>
    );
