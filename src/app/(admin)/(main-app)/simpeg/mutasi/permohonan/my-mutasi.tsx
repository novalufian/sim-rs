...
import { usePermohonanMutasiList, PermohonanMutasiWithRelations, PermohonanMutasiFilters } from '@/hooks/fetch/mutasi/useMutasiPermohonan';
import { 
    FiCalendar, 
    FiClock, 
    FiFileText, 
...
import SelectedMutasi from "@/app/(admin)/(main-app)/simpeg/mutasi/data/selectedMutasi";
import { useAppSelector } from "@/hooks/useAppDispatch";
import type { RootState } from '@/libs/store';

// Tipe untuk kolom tabel
interface MutasiColumn {
    id: keyof PermohonanMutasiWithRelations | 'actions' | 'no' | 'instansi_tujuan';
    label: string;
    width: string;
    sticky?: 'left' | 'right';
}
...
const MUTASI_TABLE_COLUMNS: MutasiColumn[] = [
    ...
    { id: 'instansi_tujuan', label: 'Instansi Tujuan', width: 'w-[250px]' },
...
const getColumnValue = (item: PermohonanMutasiWithRelations, columnId: string | keyof PermohonanMutasiWithRelations, index: number, startIndex: number) => {
    switch (columnId) {
        case 'no':
            return startIndex + index + 1;
        case 'instansi_tujuan':
            return (
                <div className="max-w-[250px] truncate" title={item.instansi_tujuan || '-'}>
        ...
        default:
            const value = item[columnId as keyof PermohonanMutasiWithRelations];
...
export default function MyMutasi() {
    const user = useAppSelector((state: RootState) => state.auth.user);
    const [page, setPage] = useState(1);
    const [statusFilter, setStatusFilter] = useState<string>('all');
    const [selectedItem, setSelectedItem] = useState<PermohonanMutasiWithRelations | null>(null);
    const [dropdownStates, setDropdownStates] = useState<Record<number, boolean>>({});
    const [drawerOpen, setDrawerOpen] = useState(false);
    const [filterDropdownOpen, setFilterDropdownOpen] = useState(false);

    // Build filters
    const filters: PermohonanMutasiFilters = useMemo(() => {
        const baseFilters: PermohonanMutasiFilters = {
            page,
    ...
    // Fetch my mutasi list
    const { data, isLoading, error } = usePermohonanMutasiList(filters);
    ...
    const handleView = (item: PermohonanMutasiWithRelations) => {
        ...
        return (
            <div className="flex justify-center items-center min-h-screen">
                <div className="text-center">
                    ...
                    <p className="text-gray-600 dark:text-gray-400">Memuat data permohonan mutasi...</p>
                </div>
            </div>
        );
    }
    ...
    if (error) {
        return (
            <div className="flex justify-center items-center min-h-screen">
                <div className="text-center">
                    <p className="text-red-600 dark:text-red-400 mb-4">❌ Gagal memuat data permohonan mutasi</p>
                </div>
            </div>
        );
    }
        ...
        <div className="container mx-auto p-4">
            ...
            <div className="mb-6">
                <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">
                    📋 Permohonan Mutasi Saya
                </h1>
                <p className="text-gray-600 dark:text-gray-400">
                    Berikut ini adalah daftar permohonan mutasi yang telah Anda ajukan
                </p>
            </div>
                        ...
                        <div className="min-table-width">
                            <table className="w-full divide-y divide-gray-200 dark:divide-gray-700">
                                <thead className="bg-gray-100 dark:bg-gray-700">
                                    <tr>
                                        {MUTASI_TABLE_COLUMNS.map((column) => (
                                            <th
                                                key={column.id}
                                                scope="col"
            ...
            <LeftDrawer 
                ...
                title="Detail Permohonan Mutasi"
            >
                <SelectedMutasi mutasi={selectedItem} />
            </LeftDrawer>
        </div>
    );
}
