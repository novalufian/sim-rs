...
import { MyApprovalMutasiItem } from "@/hooks/fetch/mutasi/useMutasiPersetujuan";
import { FiCalendar, FiFileText, FiUser, FiMapPin, FiCheckCircle, FiXCircle, FiMinusCircle, FiClock } from "react-icons/fi";

interface SelectedMyApprovalProps {
    item: MyApprovalMutasiItem | null;
    onAction?: (item: MyApprovalMutasiItem, action: 'DISETUJUI' | 'DITOLAK' | 'DIREVISI') => void;
}
...
export default function SelectedMyApproval({ item, onAction }: SelectedMyApprovalProps) {
    if (!item) {
        return <div className="p-4 text-center text-gray-500">Pilih permohonan mutasi untuk melihat detail.</div>;
    }
    ...
    return (
        <div className="space-y-6 p-4">
            ...
            {/* Detail Permohonan Mutasi */}
            <div className="space-y-4">
                <h3 className="text-xl font-semibold text-gray-900 dark:text-white border-b pb-2 dark:border-gray-700">Detail Permohonan Mutasi</h3>
                
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4"> 
                    <DetailItem icon={<FiCalendar />} label="Tanggal Pengajuan">
                        {formatDate(item.tanggal_pengajuan)}
                    </DetailItem>
                    <DetailItem icon={<FiMapPin />} label="Instansi Tujuan">
                        {item.instansi_tujuan || '-'}
                    </DetailItem>
                    {item.jenis_mutasi && (
                        <DetailItem icon={<FiFileText />} label="Jenis Mutasi">
                            {item.jenis_mutasi}
                        </DetailItem>
                    )}
                    <DetailItem icon={<FiFileText />} label="Alasan Mutasi" fullWidth>
