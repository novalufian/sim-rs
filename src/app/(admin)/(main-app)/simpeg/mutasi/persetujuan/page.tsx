...
import { useMyPersetujuanMutasiList, useProcessPersetujuanMutasi, MyApprovalMutasiItem } from '@/hooks/fetch/mutasi/useMutasiPersetujuan';
import { 
    FiCalendar, 
    FiClock, 
    FiFileText, 
...
interface ApprovalColumn {
    id: keyof MyApprovalMutasiItem | 'actions' | 'no' | 'pemohon' | 'instansi_tujuan';
    label: string;
    width: string;
    sticky?: 'left' | 'right';
}
...
const getColumnValue = (item: MyApprovalMutasiItem, columnId: string, index: number, startIndex: number) => {
    switch (columnId) {
        case 'no':
        ...
        default:
            const value = item[columnId as keyof MyApprovalMutasiItem];
...
export default function MyApprovalMutasi() {
    const [page, setPage] = useState(1);
    const [statusFilter, setStatusFilter] = useState<'ditolak' | 'diterima' | 'all'>('all');
    const [selectedItem, setSelectedItem] = useState<MyApprovalMutasiItem | null>(null);
    ...
    const { data, isLoading, error } = useMyPersetujuanMutasiList({
        page,
        limit: ITEMS_PER_PAGE,
        status: statusFilter,
    });

    const processMutation = useProcessPersetujuanMutasi();
    ...
    const handleView = (item: MyApprovalMutasiItem) => {
    ...
    const handleAction = (item: MyApprovalMutasiItem, action: 'DISETUJUI' | 'DITOLAK' | 'DIREVISI') => {
        setSelectedItem(item);
        ...
        <div className="container mx-auto p-4">
            ...
            <div className="mb-6">
                ...
                <p className="text-gray-600 dark:text-gray-400">
                    Berikut ini adalah daftar tunggu mutasi yang memerlukan <br /> persetujuan dari anda
                </p>
            </div>
                                ...
                                <tbody className="bg-white text-gray-700 dark:bg-gray-900 dark:text-gray-200 divide-y divide-gray-200 dark:divide-gray-700">
                                    {items.length === 0 ? (
                                        <tr>
                                            ...
                                            >
                                                Tidak ada permohonan mutasi yang perlu disetujui
                                            </td>
                                        </tr>
                                    ) : (
            ...
            <LeftDrawer 
                ...
                title="Detail Persetujuan Mutasi"
            >
                <SelectedMyApproval 
                    item={selectedItem} 
                    onAction={(item: MyApprovalMutasiItem, action: 'DISETUJUI' | 'DITOLAK' | 'DIREVISI') => {
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                    <div className="bg-white dark:bg-gray-900 rounded-2xl p-6 max-w-md w-full">
                        ...
                        <p className="text-gray-600 dark:text-gray-400 mb-4">
                            Apakah Anda yakin ingin {actionType === 'DISETUJUI' ? 'menyetujui' : actionType === 'DITOLAK' ? 'menolak' : 'merevisi'} permohonan mutasi ini?
                        </p>
                        <div className="mb-4">
