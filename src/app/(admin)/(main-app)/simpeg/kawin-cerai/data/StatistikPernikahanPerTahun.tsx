...
import { useStatistikPernikahanPerTahun, StatistikTahunFilter } from "@/hooks/fetch/kawin/useKawinState";

export default function StatistikPernikahanPerTahun() {
    const [filters, setFilters] = useState<StatistikTahunFilter>({});
    const { data, isLoading, isError, error } = useStatistikPernikahanPerTahun(filters);
    ...
    if (isLoading) {
        return (
            <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg">
                <h2 className="text-2xl font-bold mb-6 text-gray-900 dark:text-white">Statistik Pernikahan Per Tahun</h2>
                        ...
                        <table className="w-full divide-y divide-gray-200 dark:divide-gray-700">
                            <thead className="bg-gray-100 dark:bg-gray-700">
                                <tr>
                                    ...
                                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-white uppercase">Pasangan Pria</th>
                                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-white uppercase">Pasangan Wanita</th>
                                </tr>
                            </thead>
                            <tbody className="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
                                {statistik.per_tahun.map((item, index) => (
                                    <tr key={index} className="hover:bg-gray-50 dark:hover:bg-gray-800">
                                        ...
                                        <td className="px-4 py-3 text-sm text-gray-900 dark:text-white">{item.pasangan_pria}</td>
                                        <td className="px-4 py-3 text-sm text-gray-900 dark:text-white">{item.pasangan_wanita}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
