...
import { RiwayatPerceraianWithRelations } from "@/hooks/fetch/kawin/useKawinPermohonan";

interface SelectedPerceraianProps {
    perceraian: RiwayatPerceraianWithRelations | null;
}
...
export default function SelectedPerceraian({ perceraian }: SelectedPerceraianProps) {
    if (!perceraian) {
        return (
            <div className="p-4 text-center text-gray-500">
                Pilih riwayat perceraian untuk melihat detail
            </div>
        );
    }
            ...
            <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-4 bg-gray-50 dark:bg-gray-800/50">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <p className="text-gray-900 dark:text-white font-medium">{perceraian.pegawai_nama || '-'}</p>
                    </div>
                    ...
                    <div>
                        ...
                        <p className="text-gray-900 dark:text-white">{perceraian.pegawai_nip || '-'}</p>
                    </div>
                </div>
            </div>

            {/* Section: Detail Perceraian */}
            <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-4 bg-gray-50 dark:bg-gray-800/50">
                <h3 className="text-sm font-semibold text-gray-500 dark:text-gray-400 mb-3 uppercase tracking-wide">
                    Detail Perceraian
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <div>{renderStatusBadge(perceraian.status_saat_ini)}</div>
                    </div>
                    ...
                    <div>
                        ...
                        <p className="text-gray-900 dark:text-white">{formatDate(perceraian.tanggal_cerai)}</p>
                    </div>

                    {perceraian.alasan_perceraian && (
                        <div>
                            <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">
                                Alasan Perceraian
                            </label>
                            <p className="text-gray-900 dark:text-white">{perceraian.alasan_perceraian}</p>
                        </div>
                    )}

                    {perceraian.dokumen_akta_cerai_no && (
                        <div>
                            <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">
                                No Akta Cerai
                            </label>
                            <p className="text-gray-900 dark:text-white">{perceraian.dokumen_akta_cerai_no}</p>
                        </div>
                    )}
                </div>
            </div>
