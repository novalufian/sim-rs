...
import { useRiwayatPernikahanPegawai, StatistikPegawaiFilter } from "@/hooks/fetch/kawin/useKawinState";

export default function RiwayatPernikahanPegawai() {
    ...
    const { data, isLoading, isError, error } = useRiwayatPernikahanPegawai(searchFilters, !!searchFilters.id_pegawai || !!searchFilters.nip);
    ...
    if (isLoading) {
        return (
            <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg">
                <h2 className="text-2xl font-bold mb-6 text-gray-900 dark:text-white">Riwayat Pernikahan Pegawai</h2>
        ...
        <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg">
                    ...
                    <table className="w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead className="bg-gray-100 dark:bg-gray-700">
                            <tr>
                                ...
                                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-white uppercase">Nama Pasangan</th>
                                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-white uppercase">Tanggal Menikah</th>
                        ...
                        <tbody className="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
                            {riwayat.map((item, index) => (
                                <tr key={index} className="hover:bg-gray-50 dark:hover:bg-gray-800">
                                    ...
                                    <td className="px-4 py-3 text-sm text-gray-900 dark:text-white">{item.pasangan_nama}</td>
                                    <td className="px-4 py-3 text-sm text-gray-900 dark:text-white">{formatDate(item.tanggal_menikah)}</td>
            ...
            {riwayat && riwayat.length === 0 && (searchFilters.id_pegawai || searchFilters.nip) && (
                <div className="text-center py-8 text-gray-500">
                    Tidak ada data riwayat pernikahan untuk pegawai yang dicari.
                </div>
            )}
        </div>
    );
