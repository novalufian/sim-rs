...
export default function StatistikPernikahanPerTahunChart() {
    const [filters, setFilters] = useState<StatistikTahunFilter>({});
    const { data, isLoading, isError } = useStatistikPernikahanPerTahun(filters);
    ...
    if (isLoading) {
        return (
            <div className="rounded-4xl bg-white dark:border-gray-800 dark:bg-white/[0.03] box-border p-5 min-h-[360px] h-full flex flex-col items-center justify-center">
                <SpinerLoading title="Statistik Pernikahan Per Tahun" />
            </div>
        );
    }
    ...
    const chartData = {
        ...
        datasets: [
            {
                label: 'Total',
                data: statistik.per_tahun.map((item) => item.total),
            ...
            {
                label: 'Pasangan Pria',
                data: statistik.per_tahun.map((item) => item.pasangan_pria),
            ...
            },
            {
                ...
                data: statistik.per_tahun.map((item) => item.pasangan_wanita),
    ...
    return (
        <div className="rounded-4xl bg-white dark:border-gray-800 dark:bg-white/[0.03] box-border p-5 min-h-[360px] h-full flex flex-col">
            ...
            <h2 className="text-2xl font-semibold mb-6 text-center text-gray-800 dark:text-gray-200">Statistik Pernikahan Per Tahun</h2>
