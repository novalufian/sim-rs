...
export default function DistribusiStatusPernikahanChart() {
    const { data, isLoading, isError } = useDistribusiStatusPernikahan();
    ...
    if (isLoading) {
        return (
            <div className="rounded-4xl bg-white dark:border-gray-800 dark:bg-white/[0.03] box-border p-5 min-h-[360px] h-full flex flex-col items-center justify-center">
                <SpinerLoading title="Distribusi Status Pernikahan" />
            </div>
        );
    }
    ...
    return (
        <div className="rounded-4xl bg-white dark:border-gray-800 dark:bg-white/[0.03] box-border p-5 min-h-[360px] h-full flex flex-col items-center justify-start">
            ...
            <h2 className="text-2xl font-semibold mb-6 text-center text-gray-800 dark:text-gray-200">Distribusi Status Pernikahan</h2>
            <div className="w-full h-[260px] flex items-center justify-center">
                <Doughnut data={chartData} options={options as any} />
            </div>
            <div className="mt-4 text-center">
                <p className="text-sm text-gray-500 dark:text-gray-400">
                    Total Pegawai: <span className="font-bold text-blue-600 dark:text-blue-400">{statistik.total_pegawai}</span>
                </p>
            </div>
        </div>
    );
