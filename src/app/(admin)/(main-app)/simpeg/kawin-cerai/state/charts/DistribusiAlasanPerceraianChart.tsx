...
export default function DistribusiAlasanPerceraianChart() {
    const { data, isLoading, isError } = useDistribusiAlasanPerceraian();
    ...
    if (isLoading) {
        return (
            <div className="rounded-4xl bg-white dark:border-gray-800 dark:bg-white/[0.03] box-border p-5 min-h-[360px] h-full flex flex-col items-center justify-center">
                <SpinerLoading title="Distribusi Alasan Perceraian" />
            </div>
        );
    }
    ...
    const chartData = {
        labels: statistik.per_alasan.map((item) => item.alasan),
        datasets: [
            {
                data: statistik.per_alasan.map((item) => item.jumlah),
                backgroundColor: ["#ef4444", "#f59e0b", "#8b5cf6", "#ec4899", "#14b8a6", "#6366f1"],
                borderWidth: 1,
                borderRadius: 10,
                spacing: 5,
            },
            ...
            tooltip: {
                callbacks: {
                    label: function(context: any) {
                        ...
                        const item = statistik.per_alasan[context.dataIndex];
                        return `${item.alasan}: ${value} (${item.persentase}%)`;
                    }
                }
            }
        },
    ...
    return (
        <div className="rounded-4xl bg-white dark:border-gray-800 dark:bg-white/[0.03] box-border p-5 min-h-[360px] h-full flex flex-col items-center justify-start">
            ...
            <h2 className="text-2xl font-semibold mb-6 text-center text-gray-800 dark:text-gray-200">Distribusi Alasan Perceraian</h2>
            <div className="w-full h-[260px] flex items-center justify-center">
                <Pie data={chartData} options={options as any} />
            </div>
            <div className="mt-4 text-center">
                <p className="text-sm text-gray-500 dark:text-gray-400">
                    Total Perceraian: <span className="font-bold text-red-600 dark:text-red-400">{statistik.total_perceraian}</span>
                </p>
