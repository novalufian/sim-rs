...
import { useMyPersetujuanPensiunList, useProcessPersetujuanPensiun, MyApprovalPensiunItem } from '@/hooks/fetch/pensiun/usePensiunPersetujuan';
import { 
    FiCalendar, 
    FiClock, 
    FiFileText, 
...
interface ApprovalColumn {
    id: keyof MyApprovalPensiunItem | 'actions' | 'no' | 'pemohon';
    label: string;
    width: string;
    sticky?: 'left' | 'right';
}
...
const getColumnValue = (item: MyApprovalPensiunItem, columnId: string, index: number, startIndex: number) => {
    switch (columnId) {
        case 'no':
        ...
        default:
            const value = item[columnId as keyof MyApprovalPensiunItem];
...
export default function MyPersetujuanPensiun() {
    const [page, setPage] = useState(1);
    const [statusFilter, setStatusFilter] = useState<'ditolak' | 'diterima' | 'all'>('all');
    const [selectedItem, setSelectedItem] = useState<MyApprovalPensiunItem | null>(null);
    ...
    const { data, isLoading, error } = useMyPersetujuanPensiunList({
        page,
        limit: ITEMS_PER_PAGE,
        status: statusFilter,
    });

    const processMutation = useProcessPersetujuanPensiun();
    ...
    const handleView = (item: MyApprovalPensiunItem) => {
    ...
    const handleAction = (item: MyApprovalPensiunItem, action: 'DISETUJUI' | 'DITOLAK' | 'DIREVISI') => {
        setSelectedItem(item);
        ...
        <div className="container mx-auto p-4">
            ...
            <div className="mb-6">
                ...
                <p className="text-gray-600 dark:text-gray-400">
                    Berikut ini adalah daftar tunggu pensiun yang memerlukan <br /> persetujuan dari anda
                </p>
            </div>
                                ...
                                <tbody className="bg-white text-gray-700 dark:bg-gray-900 dark:text-gray-200 divide-y divide-gray-200 dark:divide-gray-700">
                                    {items.length === 0 ? (
                                        <tr>
                                            ...
                                            >
                                                Tidak ada permohonan pensiun yang perlu disetujui
                                            </td>
                                        </tr>
                                    ) : (
            ...
            <LeftDrawer 
                ...
                title="Detail Persetujuan Pensiun"
            >
                <SelectedMyApproval 
                    item={selectedItem} 
                    onAction={(item: MyApprovalPensiunItem, action: 'DISETUJUI' | 'DITOLAK' | 'DIREVISI') => {
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                    <div className="bg-white dark:bg-gray-900 rounded-2xl p-6 max-w-md w-full">
                        ...
                        <p className="text-gray-600 dark:text-gray-400 mb-4">
                            Apakah Anda yakin ingin {actionType === 'DISETUJUI' ? 'menyetujui' : actionType === 'DITOLAK' ? 'menolak' : 'merevisi'} permohonan pensiun ini?
                        </p>
                        <div className="mb-4">
