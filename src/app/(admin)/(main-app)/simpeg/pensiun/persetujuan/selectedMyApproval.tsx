...
import { MyApprovalPensiunItem } from "@/hooks/fetch/pensiun/usePensiunPersetujuan";
import { FiCalendar, FiFileText, FiUser, FiCheckCircle, FiXCircle, FiMinusCircle, FiClock } from "react-icons/fi";

interface SelectedMyApprovalProps {
    item: MyApprovalPensiunItem | null;
    onAction?: (item: MyApprovalPensiunItem, action: 'DISETUJUI' | 'DITOLAK' | 'DIREVISI') => void;
}
...
export default function SelectedMyApproval({ item, onAction }: SelectedMyApprovalProps) {
    if (!item) {
        return <div className="p-4 text-center text-gray-500">Pilih permohonan pensiun untuk melihat detail.</div>;
    }
    ...
    return (
        <div className="space-y-6 p-4">
            ...
            {/* Detail Permohonan Pensiun */}
            <div className="space-y-4">
                <h3 className="text-xl font-semibold text-gray-900 dark:text-white border-b pb-2 dark:border-gray-700">Detail Permohonan Pensiun</h3>
                
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4"> 
                    <DetailItem icon={<FiCalendar />} label="Tanggal Pengajuan">
                        {formatDate(item.tanggal_pengajuan)}
                    </DetailItem>
                    <DetailItem icon={<FiCalendar />} label="Tanggal Pensiun">
                        {formatDate(item.tanggal_pensiun)}
                    </DetailItem>
                    <DetailItem icon={<FiFileText />} label="Jenis Pensiun">
                        {item.jenis_pensiun || '-'}
                    </DetailItem>
                    <DetailItem icon={<FiFileText />} label="Alasan Pensiun" fullWidth>
