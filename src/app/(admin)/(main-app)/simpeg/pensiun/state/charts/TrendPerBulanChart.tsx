...
import { useTrendPerBulan, StatistikBulanFilter } from "@/hooks/fetch/pensiun/usePensiunState";
...
export default function TrendPerBulanChart() {
    ...
    const chartData = {
        labels: statistik.data.map((item) => item.bulan),
        datasets: [
            {
                label: 'Pengajuan',
                data: statistik.data.map((item) => item.pengajuan),
            ...
            {
                label: 'Selesai',
                data: statistik.data.map((item) => item.selesai),
            ...
            {
                label: 'Pending',
                data: statistik.data.map((item) => item.pending),
        ...
        <div className="rounded-4xl bg-white dark:border-gray-800 dark:bg-white/[0.03] box-border p-5 min-h-[360px] h-full flex flex-col">
            ...
            <div className="mt-4 text-center">
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400">
                    Total Pengajuan: {statistik.total_pengajuan} | Total Selesai: {statistik.total_selesai}
                </p>
            </div>
        </div>
    );
