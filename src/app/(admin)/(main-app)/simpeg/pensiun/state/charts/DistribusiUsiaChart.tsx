...
import { useDistribusiUsia } from "@/hooks/fetch/pensiun/usePensiunState";
...
export default function DistribusiUsiaChart() {
    const { data, isLoading, isError } = useDistribusiUsia();
    ...
    if (isLoading) {
        return (
            <div className="rounded-4xl bg-white dark:border-gray-800 dark:bg-white/[0.03] box-border p-5 min-h-[360px] h-full flex flex-col items-center justify-center">
                <SpinerLoading title="Distribusi Usia" />
            </div>
        );
    }
    ...
    const chartData = {
        labels: statistik.distribusi.map((item) => `Usia ${item.usia}`),
        datasets: [
            {
                label: 'Jumlah Pegawai',
                data: statistik.distribusi.map((item) => item.jumlah),
    ...
    return (
        <div className="rounded-4xl bg-white dark:border-gray-800 dark:bg-white/[0.03] box-border p-5 min-h-[360px] h-full flex flex-col">
            ...
            <h2 className="text-2xl font-semibold mb-6 text-center text-gray-800 dark:text-gray-200">Distribusi Usia Pegawai</h2>
            <div className="w-full h-[260px]">
                <Bar data={chartData} options={options as any} />
            </div>
            <div className="mt-4 text-center">
                <p className="text-sm text-gray-500 dark:text-gray-400">
                    Total Pegawai: <span className="font-bold text-blue-600 dark:text-blue-400">{statistik.total}</span>
                </p>
            </div>
        </div>
    );
