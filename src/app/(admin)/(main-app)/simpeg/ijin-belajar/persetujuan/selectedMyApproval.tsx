...
import { MyApprovalBelajarItem } from "@/hooks/fetch/belajar/useBelajarPersetujuan";
import { FiCalendar, FiFileText, FiUser, FiBook, FiCheckCircle, FiXCircle, FiMinusCircle, FiClock } from "react-icons/fi";

interface SelectedMyApprovalProps {
    item: MyApprovalBelajarItem | null;
    onAction?: (item: MyApprovalBelajarItem, action: 'DISETUJUI' | 'DITOLAK' | 'DIREVISI') => void;
}
...
export default function SelectedMyApproval({ item, onAction }: SelectedMyApprovalProps) {
    if (!item) {
        return <div className="p-4 text-center text-gray-500">Pilih permohonan ijin belajar untuk melihat detail.</div>;
    }
    ...
    return (
        <div className="space-y-6 p-4">
            ...
            {/* Detail Permohonan Ijin Belajar */}
            <div className="space-y-4">
                <h3 className="text-xl font-semibold text-gray-900 dark:text-white border-b pb-2 dark:border-gray-700">Detail Permohonan Ijin Belajar</h3>
                
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4"> 
                    <DetailItem icon={<FiBook />} label="Nama Sekolah">
                        {item.nama_sekolah || '-'}
                    </DetailItem>
                    <DetailItem icon={<FiFileText />} label="Jenjang Pendidikan">
                        {item.jenjang_pendidikan || '-'}
                    </DetailItem>
                    {item.jurusan && (
                        <DetailItem icon={<FiFileText />} label="Jurusan">
                            {item.jurusan}
                        </DetailItem>
                    )}
                    <DetailItem icon={<FiCalendar />} label="Tanggal Mulai">
                        {formatDate(item.tanggal_mulai)}
                    </DetailItem>
                    {item.tanggal_selesai && (
                        <DetailItem icon={<FiCalendar />} label="Tanggal Selesai">
                            {formatDate(item.tanggal_selesai)}
                        </DetailItem>
                    )}
                    {item.alamat_sekolah && (
                        <DetailItem icon={<FiFileText />} label="Alamat Sekolah" fullWidth>
                            {item.alamat_sekolah}
                        </DetailItem>
                    )}
                </div>
