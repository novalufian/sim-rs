...
import { useMyPersetujuanBelajarList, useProcessPersetujuanBelajar, MyApprovalBelajarItem } from '@/hooks/fetch/belajar/useBelajarPersetujuan';
import { 
    FiCalendar, 
    FiClock, 
    FiFileText, 
...
interface ApprovalColumn {
    id: keyof MyApprovalBelajarItem | 'actions' | 'no' | 'pemohon' | 'institusi_info';
    label: string;
    width: string;
    sticky?: 'left' | 'right';
}
...
const APPROVAL_TABLE_COLUMNS: ApprovalColumn[] = [
    ...
    { id: 'tanggal_mulai', label: 'Tgl Mulai', width: 'w-[150px]' },
...
const getColumnValue = (item: MyApprovalBelajarItem, columnId: string, index: number, startIndex: number) => {
    switch (columnId) {
        case 'no':
        ...
        case 'institusi_info':
            return (
                <div className="max-w-[250px] truncate" title={item.nama_sekolah || '-'}>
                    <span className="text-sm font-medium text-gray-900 dark:text-white">
                        {item.nama_sekolah || '-'}
                    </span>
                </div>
        ...
        case 'tanggal_mulai':
            return formatDate(item.tanggal_mulai);
        case 'status_persetujuan':
            return renderStatusBadge(item.status_persetujuan);
        default:
            const value = item[columnId as keyof MyApprovalBelajarItem];
...
export default function MyPersetujuanBelajar() {
    const [page, setPage] = useState(1);
    const [statusFilter, setStatusFilter] = useState<'ditolak' | 'diterima' | 'all'>('all');
    const [selectedItem, setSelectedItem] = useState<MyApprovalBelajarItem | null>(null);
    ...
    const { data, isLoading, error } = useMyPersetujuanBelajarList({
        page,
        limit: ITEMS_PER_PAGE,
        status: statusFilter,
    });

    const processMutation = useProcessPersetujuanBelajar();
    ...
    const handleView = (item: MyApprovalBelajarItem) => {
    ...
    const handleAction = (item: MyApprovalBelajarItem, action: 'DISETUJUI' | 'DITOLAK' | 'DIREVISI') => {
        setSelectedItem(item);
        ...
        <div className="container mx-auto p-4">
            ...
            <div className="mb-6">
                ...
                <p className="text-gray-600 dark:text-gray-400">
                    Berikut ini adalah daftar tunggu ijin belajar yang memerlukan <br /> persetujuan dari anda
                </p>
            </div>
                                ...
                                <tbody className="bg-white text-gray-700 dark:bg-gray-900 dark:text-gray-200 divide-y divide-gray-200 dark:divide-gray-700">
                                    {items.length === 0 ? (
                                        <tr>
                                            ...
                                            >
                                                Tidak ada permohonan ijin belajar yang perlu disetujui
                                            </td>
                                        </tr>
                                    ) : (
            ...
            <LeftDrawer 
                ...
                title="Detail Persetujuan Ijin Belajar"
            >
                <SelectedMyApproval 
                    item={selectedItem} 
                    onAction={(item: MyApprovalBelajarItem, action: 'DISETUJUI' | 'DITOLAK' | 'DIREVISI') => {
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                    <div className="bg-white dark:bg-gray-900 rounded-2xl p-6 max-w-md w-full">
                        ...
                        <p className="text-gray-600 dark:text-gray-400 mb-4">
                            Apakah Anda yakin ingin {actionType === 'DISETUJUI' ? 'menyetujui' : actionType === 'DITOLAK' ? 'menolak' : 'merevisi'} permohonan ijin belajar ini?
                        </p>
                        <div className="mb-4">
