...
import { usePermohonanPerUnitKerja, StatistikUnitKerjaFilter } from "@/hooks/fetch/belajar/useBelajarState";
...
export default function PermohonanPerUnitKerjaChart() {
    const [filters, setFilters] = useState<StatistikUnitKerjaFilter>({});
    const { data, isLoading, isError } = usePermohonanPerUnitKerja(filters);
    ...
    if (isLoading) {
        return (
            <div className="rounded-4xl bg-white dark:border-gray-800 dark:bg-white/[0.03] box-border p-5 min-h-[360px] h-full flex flex-col items-center justify-center">
                <SpinerLoading title="Permohonan Per Unit Kerja" />
            </div>
        );
    }
    ...
    const top10 = statistik.per_unit_kerja.slice(0, 10);

    const chartData = {
        labels: top10.map((item) => item.nama_unit_kerja.length > 20 ? item.nama_unit_kerja.substring(0, 20) + '...' : item.nama_unit_kerja),
        datasets: [
            {
                label: 'Jumlah Permohonan',
                data: top10.map((item) => item.jumlah_permohonan),
    ...
    return (
        <div className="rounded-4xl bg-white dark:border-gray-800 dark:bg-white/[0.03] box-border p-5 min-h-[360px] h-full flex flex-col">
            ...
            <h2 className="text-2xl font-semibold mb-6 text-center text-gray-800 dark:text-gray-200">Permohonan Per Unit Kerja (Top 10)</h2>
            <div className="w-full h-[260px]">
                <Bar data={chartData} options={options as any} />
            </div>
            <div className="mt-4 text-center">
                <p className="text-sm text-gray-500 dark:text-gray-400">
                    Total Unit Kerja: <span className="font-bold text-blue-600 dark:text-blue-400">{statistik.total_unit_kerja}</span>
                </p>
