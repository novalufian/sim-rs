...
import { useTopProgramStudi } from "@/hooks/fetch/belajar/useBelajarState";
...
export default function TopProgramStudiChart() {
    const { data, isLoading, isError } = useTopProgramStudi();
    ...
    if (isLoading) {
        return (
            <div className="rounded-4xl bg-white dark:border-gray-800 dark:bg-white/[0.03] box-border p-5 min-h-[360px] h-full flex flex-col items-center justify-center">
                <SpinerLoading title="Top Program Studi" />
            </div>
        );
    }
    ...
    const top10 = statistik.top_program_studi.slice(0, 10);

    const chartData = {
        labels: top10.map((item) => item.nama_program_studi.length > 30 ? item.nama_program_studi.substring(0, 30) + '...' : item.nama_program_studi),
        datasets: [
            {
    ...
    return (
        <div className="rounded-4xl bg-white dark:border-gray-800 dark:bg-white/[0.03] box-border p-5 min-h-[360px] h-full flex flex-col">
            ...
            <h2 className="text-2xl font-semibold mb-6 text-center text-gray-800 dark:text-gray-200">Top Program Studi (Top 10)</h2>
