...
export default function TopInstitusiPendidikanChart() {
    ...
    const top10 = statistik.top_institusi.slice(0, 10);

    const chartData = {
        labels: top10.map((item) => item.nama_institusi.length > 30 ? item.nama_institusi.substring(0, 30) + '...' : item.nama_institusi),
        datasets: [
            {
    ...
    return (
        <div className="rounded-4xl bg-white dark:border-gray-800 dark:bg-white/[0.03] box-border p-5 min-h-[360px] h-full flex flex-col">
            ...
            <h2 className="text-2xl font-semibold mb-6 text-center text-gray-800 dark:text-gray-200">Top Institusi Pendidikan (Top 10)</h2>
