...
import { useProyeksi5Tahun } from "@/hooks/fetch/belajar/useBelajarState";
...
export default function Proyeksi5TahunChart() {
    ...
    const chartData = {
        ...
        datasets: [
            {
                label: 'Aktual',
                data: statistik.proyeksi.map((item) => item.tipe === 'aktual' ? item.jumlah_permohonan : null),
            ...
            {
                label: 'Proyeksi',
                data: statistik.proyeksi.map((item) => item.tipe === 'proyeksi' ? item.jumlah_permohonan : null),
