...
import { useDistribusiBiayaDitanggung } from "@/hooks/fetch/belajar/useBelajarState";
import SpinerLoading from "@/components/loading/spiner";
import { BsCloudSlash } from "react-icons/bs";

ChartJS.register(ArcElement, Tooltip, Legend);

export default function DistribusiBiayaDitanggungChart() {
    const { data, isLoading, isError } = useDistribusiBiayaDitanggung();
    ...
    if (isLoading) {
        return (
            <div className="rounded-4xl bg-white dark:border-gray-800 dark:bg-white/[0.03] box-border p-5 min-h-[360px] h-full flex flex-col items-center justify-center">
                <SpinerLoading title="Distribusi Biaya Ditanggung" />
            </div>
        );
    }
    ...
    const chartData = {
        labels: statistik.per_biaya.map((item) => item.biaya_ditanggung.replace(/_/g, ' ')),
        datasets: [
            {
                data: statistik.per_biaya.map((item) => item.jumlah),
            ...
            tooltip: {
                callbacks: {
                    label: function(context: any) {
                        ...
                        const item = statistik.per_biaya[context.dataIndex];
                        return `${item.biaya_ditanggung.replace(/_/g, ' ')}: ${value} (${item.persentase}%)`;
                    }
                }
            }
        },
    ...
    return (
        <div className="rounded-4xl bg-white dark:border-gray-800 dark:bg-white/[0.03] box-border p-5 min-h-[360px] h-full flex flex-col items-center justify-start">
            ...
            <h2 className="text-2xl font-semibold mb-6 text-center text-gray-800 dark:text-gray-200">Distribusi Biaya Ditanggung</h2>
