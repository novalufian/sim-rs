...
import { useStatistikGelarDiperoleh } from "@/hooks/fetch/belajar/useBelajarState";
import SpinerLoading from "@/components/loading/spiner";
import { BsCloudSlash } from "react-icons/bs";

ChartJS.register(ArcElement, Tooltip, Legend);

export default function StatistikGelarDiperolehChart() {
    const { data, isLoading, isError } = useStatistikGelarDiperoleh();
    ...
    if (isLoading) {
        return (
            <div className="rounded-4xl bg-white dark:border-gray-800 dark:bg-white/[0.03] box-border p-5 min-h-[360px] h-full flex flex-col items-center justify-center">
                <SpinerLoading title="Statistik Gelar Diperoleh" />
            </div>
        );
    }
    ...
    const chartData = {
        labels: statistik.per_gelar.map((item) => item.gelar),
        datasets: [
            {
                data: statistik.per_gelar.map((item) => item.jumlah),
            ...
            tooltip: {
                callbacks: {
                    label: function(context: any) {
                        ...
                        const item = statistik.per_gelar[context.dataIndex];
                        return `${item.gelar}: ${value} (${item.persentase}%)`;
                    }
                }
            }
        },
    ...
    return (
        <div className="rounded-4xl bg-white dark:border-gray-800 dark:bg-white/[0.03] box-border p-5 min-h-[360px] h-full flex flex-col items-center justify-start">
            ...
            <h2 className="text-2xl font-semibold mb-6 text-center text-gray-800 dark:text-gray-200">Statistik Gelar Diperoleh</h2>
            <div className="w-full h-[260px] flex items-center justify-center">
                <Doughnut data={chartData} options={options as any} />
            </div>
            <div className="mt-4 text-center">
                <p className="text-sm text-gray-500 dark:text-gray-400">
                    Total Gelar: <span className="font-bold text-blue-600 dark:text-blue-400">{statistik.total_gelar}</span>
                </p>
