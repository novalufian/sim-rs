...
import { MyApprovalCutiItem } from "@/hooks/fetch/cuti/useCutiPersetujuan";
import { FiCalendar, FiClock, FiFileText, FiUser, FiMapPin, FiPhone, FiCheckCircle, FiXCircle, FiMinusCircle } from "react-icons/fi";

interface SelectedMyApprovalProps {
    item: MyApprovalCutiItem | null;
    onAction?: (item: MyApprovalCutiItem, action: 'DISETUJUI' | 'DITOLAK' | 'DIREVISI') => void;
}
...
export default function SelectedMyApproval({ item, onAction }: SelectedMyApprovalProps) {
    ...
    return (
        <div className="space-y-6 p-4">
            ...
            <div className="space-y-4">
                <h3 className="text-xl font-semibold text-gray-900 dark:text-white border-b pb-2 dark:border-gray-700">Detail Permohonan Cuti</h3>
                
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4"> 
                    <DetailItem icon={<FiCalendar />} label="Periode Cuti">
                        {formatDate(item.permohonan_tanggal_mulai_cuti)} - {formatDate(item.permohonan_tanggal_selesai_cuti)}
                    </DetailItem>
                    <DetailItem icon={<FiClock />} label="Durasi">
                        {item.permohonan_jumlah_hari} hari
                    </DetailItem>
                    <DetailItem icon={<FiFileText />} label="Jenis Cuti">
                        {item.permohonan_id_jenis_cuti || '-'}
                    </DetailItem>
                    <DetailItem icon={<FiMapPin />} label="Alamat Selama Cuti">
                        {item.permohonan_alamat_selama_cuti || '-'}
                    </DetailItem>
                    {item.permohonan_no_hp_selama_cuti && (
                        <DetailItem icon={<FiPhone />} label="No. HP Selama Cuti">
                            {item.permohonan_no_hp_selama_cuti}
                        </DetailItem>
                    )}
